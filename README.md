...
1. **Padrão Facade (`TravelService`):**
   ...
   - **Problema:** a visão de um destino exige país, feriados e câmbio de fontes distintas; controllers e CLI não devem orquestrar isso nem tratar casos como país com moeda BRL.
   - **Por quê:** o `WebController` (e o CLI) não precisam conhecer as três APIs externas, nem repetir lógica de agregação e degradação em cada interface.
...
2. **Padrão Strategy (`ScoringStrategy`):**
   ...
   - **Problema:** o ranking combina critérios heterogêneos (clima, custo, distância, festividades) que evoluem em ritmos diferentes; um único método de score ficaria monolítico e difícil de testar.
   - **Por quê:** adicionar ou alterar um critério exige só uma nova implementação registrada no Spring — o motor permanece genérico. Complementa o Facade: enquanto o `TravelService` agrega dados de **um** país, o motor compara **vários** destinos com regras extensíveis.
...
| Padrão       | Classe(s)                          | Entrega | Problema que resolve                                              | Por quê adotamos                                              |
| ------------ | ---------------------------------- | ------- | ----------------------------------------------------------------- | ------------------------------------------------------------- |
| **Facade**   | `TravelService`                    | 1       | Visão de um destino exige orquestrar país, feriados e câmbio      | Controllers/CLI não repetem integração nem tratamento de falhas |
| **Strategy** | `ScoringStrategy` + implementações | 2       | Critérios de score evoluem de forma independente                  | Novo critério = nova classe; o motor permanece genérico       |
...
1. **Ports & adapters (clients concretos) e padrão Decorator (`Caching*Client`):**
   ...
   - `StaticCountryClient`, `NagerDateClient`, `AwesomeApiClient`, `OpenMeteoClient`, `WorldBankClient` e `WikipediaRestClient` implementam cada porta traduzindo a API externa para o contrato do domínio.
   - **Problema (ports & adapters):** cada provedor expõe URLs, formatos JSON e semânticas diferentes; os serviços de domínio não devem conhecer HTTP nem detalhes de integração.
   - **Por quê (ports & adapters):** isola a tradução externa → domínio em classes substituíveis, permitindo trocar provedor ou decorar com cache sem alterar `CountryService`, `ExchangeService` etc. *Não é o Adapter GoF clássico (não há Adaptee legado com interface incompatível) — segue o vocabulário de arquitetura hexagonal.*
   ...
   - **Problema (Decorator):** comparar dezenas de destinos repete consultas idênticas (feriados da origem, câmbio, clima da mesma coordenada).
   - **Por quê (Decorator):** evita chamadas externas redundantes dentro de uma mesma comparação (ex.: feriados da origem consultados uma única vez, não uma por candidato), sem acoplar a lógica de cache ao código de integração HTTP.
...
2. **Resiliência (Resilience4j) e simple factory (`RestClientFactory`):**
   ...
   - **Problema (simple factory):** seis clients HTTP precisam dos mesmos timeouts, `User-Agent` e configuração de rede; repetir esse setup em cada classe gera divergência e dificulta operação.
   - **Por quê (simple factory):** `RestClientFactory` centraliza a criação dos `RestClient` com timeout de conexão (3s) e leitura (5s) configuráveis via `app.external-apis.connect-timeout`/`read-timeout`, evitando que uma API externa lenta bloqueie a aplicação indefinidamente. *Não é Factory Method nem Abstract Factory (GoF) — apenas utilitário de criação.*
...
7. **Filtros e explicabilidade (Chain of Responsibility):**
   ...
   - **Problema:** exclusões de países, teto de orçamento terrestre e futuras restrições são regras independentes; acumular `if/else` no motor o tornaria rígido e opaco.
   - **Por quê:** cada filtro encapsula uma restrição com motivo explícito; para incluir ou remover uma regra, basta adicionar ou retirar uma classe de filtro — a cadeia se remonta sozinha, sem alterar o `TravelRecommendationEngine`.
...
4. **Padrão Facade (`DestinationProfileService`):**
   ...
   - **Problema:** enriquecer um destino exige duas integrações lentas e instáveis; o motor não deve saber de onde vêm população e conteúdo editorial.
   - **Por quê:** um único ponto de entrada com degradação isolada — falha da Wikipédia não remove população; falha demográfica não remove conteúdo editorial; falha total de enriquecimento não elimina o destino do ranking.
...
| Padrão                      | Classe(s)                                                                                                                                                    | Problema que resolve                                                                                 | Por quê adotamos                                                                                  |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **Decorator**               | `CachingCountryClient`, `CachingHolidayClient`, `CachingExchangeClient`, `CachingWeatherClient`, `CachingWorldBankIndicatorClient`, `CachingWikipediaClient` | Comparar N destinos repete consultas idênticas às mesmas fontes                                      | Cache transparente sobre a mesma interface, sem contaminar clients ou serviços                    |
| **Strategy**                | `ScoringStrategy` + 4 implementações                                                                                                                         | Critérios heterogêneos (clima, custo, distância, festividades) evoluem em ritmos diferentes          | Plug-in de regras sem modificar o motor; combinação por média ponderada                           |
| **Chain of Responsibility** | `CandidateFilter`, `ExcludedCountriesFilter`, `MaxGroundBudgetFilter`                                                                                        | Restrições obrigatórias (exclusões, orçamento) crescem e não devem virar `if/else` no motor          | Cada filtro com motivo explícito em `skipped`; nova regra = nova classe de filtro na cadeia       |
| **Facade**                  | `TravelService`                                                                                                                                              | Visão individual de destino exige agregar múltiplas fontes                                           | Uma operação para controllers/CLI/API sem orquestração espalhada                                  |
| **Facade**                  | `DestinationProfileService`                                                                                                                                  | Enriquecimento editorial e demográfico vem de integrações distintas e instáveis                    | Um `DestinationProfile` com degradação isolada por fonte                                          |

#### Integração (técnicas complementares, não GoF)

| Técnica | Classe(s) | Problema que resolve | Por quê adotamos |
|---|---|---|---|
| **Ports & adapters** | `StaticCountryClient`, `NagerDateClient`, `AwesomeApiClient`, `OpenMeteoClient`, `WorldBankClient`, `WikipediaRestClient` | Cada API externa tem contrato e formato próprios; o domínio não deve conhecer HTTP | Portas (`*Client`) + adapters secundários substituíveis; distinto do Adapter GoF clássico |
| **Simple factory** | `RestClientFactory` | Vários clients HTTP precisam da mesma configuração de rede | Timeouts e User-Agent consistentes em um único ponto; distinto de Factory Method/Abstract Factory |
