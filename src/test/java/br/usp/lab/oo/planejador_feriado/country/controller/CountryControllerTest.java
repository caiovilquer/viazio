...
class CountryControllerTest {
  ...
  void shouldReturnCountryByCode() throws Exception {
    ...
    mockMvc
      .perform(get("/api/v1/countries/JP"))
  ...
  void shouldReturn404WhenCountryNotFound() throws Exception {
    when(countryService.getCountryByCode("ZZ")).thenThrow(
      new ResourceNotFoundException("Country not found: ZZ")
    );

    mockMvc
      .perform(get("/api/v1/countries/ZZ"))
  ...
  void shouldReturn502WhenUpstreamFails() throws Exception {
    when(countryService.getCountryByCode("JP")).thenThrow(
      new ExternalApiException(
        "Falha ao consultar serviço de países",
  ...
  void shouldSearchCountryByName() throws Exception {
    ...
    mockMvc
      .perform(get("/api/v1/countries/search").param("name", "japan"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.name").value("Japan"));
  }
}
