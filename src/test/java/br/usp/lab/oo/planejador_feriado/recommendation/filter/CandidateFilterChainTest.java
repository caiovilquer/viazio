...
class CandidateFilterChainTest {
  ...
  private final Country france = new Country(
    "France",
    "FR",
    "Europe",
    "Western Europe",
  ...
  void passesWhenCountryIsAllowed() {
    assertTrue(
      chain
        .reject(new FilterContext("FR", france, null, request(List.of())))
        .isEmpty()
    );
  }
  ...
  void rejectsExcludedCountry() {
    var rejection = chain.reject(
      new FilterContext("FR", france, null, request(List.of("FR")))
    );
  ...
  void rejectsEstimateAboveGroundBudget() {
    ...
    var rejection = chain.reject(
      new FilterContext("FR", france, estimate, request(List.of(), 4_000.0))
    );
