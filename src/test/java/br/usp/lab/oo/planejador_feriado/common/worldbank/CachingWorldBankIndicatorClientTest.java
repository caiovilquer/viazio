...
class CachingWorldBankIndicatorClientTest {
  ...
  void cachesRepeatedCallsByIsoCodeAndIndicator() {
    List<WorldBankIndicatorPoint> series = List.of(
      new WorldBankIndicatorPoint("2023", 216_000_000.0)
    );
    when(delegate.getIndicatorSeries("BR", "SP.POP.TOTL")).thenReturn(series);

    CachingWorldBankIndicatorClient client =
      new CachingWorldBankIndicatorClient(delegate);

    List<WorldBankIndicatorPoint> first = client.getIndicatorSeries(
      "BR",
      "SP.POP.TOTL"
    );
    List<WorldBankIndicatorPoint> second = client.getIndicatorSeries(
      "br",
      "SP.POP.TOTL"
    );
  ...
  void treatsDifferentIndicatorsAsDifferentCacheEntries() {
    List<WorldBankIndicatorPoint> population = List.of(
      new WorldBankIndicatorPoint("2023", 216_000_000.0)
    );
    List<WorldBankIndicatorPoint> ppp = List.of(
      new WorldBankIndicatorPoint("2021", 43.0)
    );
    when(delegate.getIndicatorSeries("BR", "SP.POP.TOTL")).thenReturn(
      population
    );
