...
public final class LongWeekendDetector {
  ...
  public List<LongWeekend> detect(
    List<Holiday> holidays,
    LocalDate from,
    LocalDate to
  ) {
    List<Holiday> publicHolidays = holidays
      .stream()
    ...
    Set<LocalDate> holidayDates = publicHolidays
      .stream()
      .map(Holiday::getDate)
    ...
    for (Holiday holiday : publicHolidays) {
      ...
      if (totalDays >= 3 || bridgeDays > 0) {
        result.add(
          new LongWeekend(start, end, totalDays, bridgeDays, holiday.getName())
        );
  ...
  private LocalDate expandWeekendsBackward(LocalDate start, LocalDate from) {
    while (
      isWeekend(start.minusDays(1)) && !start.minusDays(1).isBefore(from)
    ) {
  ...
  private boolean canBridgeBefore(
    LocalDate bridgeDay,
    LocalDate from,
    Set<LocalDate> holidayDates
  ) {
    if (
      bridgeDay.isBefore(from) ||
      isWeekend(bridgeDay) ||
      holidayDates.contains(bridgeDay)
    ) {
      return false;
    }
    ...
    return (
      ...
      (isWeekend(anchor) || holidayDates.contains(anchor))
    );
  }

  private boolean canBridgeAfter(
    LocalDate bridgeDay,
    LocalDate to,
    Set<LocalDate> holidayDates
  ) {
    if (
      bridgeDay.isAfter(to) ||
      isWeekend(bridgeDay) ||
      holidayDates.contains(bridgeDay)
    ) {
      return false;
