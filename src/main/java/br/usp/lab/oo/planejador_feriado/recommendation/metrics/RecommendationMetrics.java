...
public class RecommendationMetrics {
  ...
  public RecommendationMetrics(MeterRegistry registry) {
    ...
    this.evaluatedCandidates = DistributionSummary.builder(
      "travel.recommendation.candidates.evaluated"
    )
      .description("Quantidade de candidatos avaliados por busca")
      .register(registry);
    this.returnedRecommendations = DistributionSummary.builder(
      "travel.recommendation.results.returned"
    )
      .description("Quantidade de recomendações devolvidas por busca")
      .register(registry);
  }

  public RecommendationResponse measure(
    Supplier<RecommendationResponse> operation
  ) {
    ...
    try {
      ...
      registry
        .counter("travel.recommendation.requests", "outcome", outcome)
        .increment();
      return response;
