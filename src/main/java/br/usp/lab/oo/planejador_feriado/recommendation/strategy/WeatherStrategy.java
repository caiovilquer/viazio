  ...
  public ScoreEntry evaluate(RecommendationContext context) {
    ...
    if (weather == null) {
      return ScoreEntry.unavailable(
        criterion(),
        "Clima indisponível para o destino"
      );
    }

    double tempScore = temperatureScore(weather.avgTempC());
    double rainAmountScore = Math.max(
      0.0,
      100.0 - weather.avgDailyPrecipMm() * 10.0
    );
    double rainyDaysScore = Math.max(
      0.0,
      100.0 - weather.rainyDayProbability() * 100.0
    );
    ...
    double score =
      0.40 * tempScore + 0.50 * precipitationScore + 0.10 * stabilityScore;
    ...
    String justification = String.format(
      ...
      weather.sourceType() ==
        br.usp.lab.oo.planejador_feriado.weather.model.WeatherSourceType.FORECAST
        ? "previsão"
        : "climatologia",
