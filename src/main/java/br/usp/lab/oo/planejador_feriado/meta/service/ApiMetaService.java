...
public class ApiMetaService {
  ...
  private ApiMetaResponse build() {
    ...
    List<CriterionOption> criteria = List.of(Criterion.values())
      .stream()
      .map(criterion ->
        new CriterionOption(
          criterion.key(),
    ...
    List<ProfileOption> profiles = weightResolver
      .availableProfiles()
      .stream()
      .map(profile ->
        new ProfileOption(
    ...
    List<CountryOption> countries = countryService
      .getAllTravelEligibleCountries()
      .stream()
      .sorted(
    ...
    return new ApiMetaResponse(
      ...
      Map.of(
        ...
        "groundBudgetFilter",
        true,
        "customOriginCoordinates",
        true,
        "liveCommercialPrices",
        false
      )
    );
  }
  ...
  private CountryOption toCountryOption(Country country) {
    List<DestinationCity> catalogCities = destinationCatalogService.getCities(
      country.getIsoCode()
    );
    List<CityOption> cities = catalogCities
      .stream()
      .map(city ->
        new CityOption(
          city.name(),
    ...
    String defaultCity = cities
      ...
      .findFirst()
      .orElseGet(() ->
        country.getCapitals().isEmpty() ? null : country.getCapitals().get(0)
      );
