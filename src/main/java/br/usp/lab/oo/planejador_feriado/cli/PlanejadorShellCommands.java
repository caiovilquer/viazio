...
public class PlanejadorShellCommands {
  ...
  @Command(
    command = "pais-por-codigo",
    description = "Busca país pelo código ISO 3166-1 alpha-2"
  )
  public String countryByCode(
    @Option(
      required = true,
      longNames = "codigo",
      description = "Ex.: BR, US"
    ) String codigo
  ) {
  ...
  @Command(
    command = "pais-por-nome",
    description = "Busca país pelo nome comum em inglês"
  )
  public String countryByName(
    @Option(
      required = true,
      longNames = "nome",
      description = "Ex.: brazil, germany"
    ) String nome
  ) {
  ...
  @Command(
    command = "feriados",
    description = "Lista feriados públicos futuros no ano atual para o país"
  )
  public String holidays(
    @Option(
      required = true,
      longNames = "codigo",
      description = "Código ISO, ex.: BR"
    ) String codigo
  ) {
    Country c = countryService.getCountryByCode(codigo.trim());
  ...
  @Command(
    command = "cambio",
    description = "Cotação: 1 unidade da moeda para BRL (AwesomeAPI)"
  )
  public String exchange(
    @Option(
      required = true,
      longNames = "moeda",
      description = "Ex.: USD, EUR"
    ) String moeda
  ) {
  ...
  @Command(
    command = "viagem",
    description = "Resumo: país + feriados futuros + câmbio quando não for BRL"
  )
  public String travel(
    ...
    if (o.exchangeToBrl() == null) {
      sb.append("(não aplicável ou indisponível)").append(
