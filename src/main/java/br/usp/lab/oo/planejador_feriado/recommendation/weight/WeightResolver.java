...
public class WeightResolver {
    ...
    public boolean isKnownProfile(String profile) {
        return profile != null && properties.profiles().containsKey(profile.toLowerCase(Locale.ROOT));
    }

    public ResolvedWeights resolve(String profile, Map<Criterion, Double> overrides) {
        String normalizedProfile = profile != null ? profile.trim().toLowerCase(Locale.ROOT) : null;

        Map<String, Double> base = properties.defaultWeights();
        boolean usingProfile = normalizedProfile != null && properties.profiles().containsKey(normalizedProfile);
        if (usingProfile) {
        ...
        base.forEach((key, value) -> Criterion.fromKey(key)
                .ifPresent(criterion -> weights.put(criterion, Math.max(0.0, value))));
        ...
        return new ResolvedWeights(resolveName(usingProfile, normalizedProfile, hasOverrides), weights);
    }
    ...
    private String resolveName(boolean usingProfile, String profile, boolean hasOverrides) {
        if (usingProfile) {
