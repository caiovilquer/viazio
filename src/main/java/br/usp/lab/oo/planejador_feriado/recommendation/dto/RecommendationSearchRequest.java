...
@Schema(
  description = "Busca estruturada de recomendações para consumo por frontend"
)
public record RecommendationSearchRequest(
  ...
  List<@NotBlank(message = "não pode ser vazio") @Pattern(
    regexp = "(?i)[A-Z]{2}",
    message = "deve usar código ISO alpha-2"
  ) String> countries,
  ...
  @Max(
    value = RecommendationLimits.MAX_RESULTS,
    message = "deve ser no máximo 15"
  )
  ...
  Map<
    @NotBlank(message = "não pode ser vazio") @Pattern(
      regexp = "(?i)(weather|cost|distance|festivities)",
      message = "critério desconhecido"
    ) String,
    @NotNull(message = "é obrigatório") @DecimalMin(
      value = "0.0",
      message = "deve ser maior ou igual a 0"
    ) @DecimalMax(value = "1.0", message = "deve ser menor ou igual a 1") Double
  > weights,
  ...
  @Valid OriginInput origin,

  @Min(value = 1, message = "deve ser no mínimo 1")
  @Max(
    value = RecommendationLimits.MAX_TRAVELERS,
    message = "deve ser no máximo 10"
  )
  ...
  public OriginInput originOrDefault() {
    return origin != null
      ? origin
      : new OriginInput("BR", null, null, null, null);
  }
}
