...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  protected ResponseEntity<Object> handleMethodArgumentNotValid(
    ...
    List<ApiViolation> violations = ex
      .getBindingResult()
      .getFieldErrors()
      .stream()
      .map(error ->
  ...
  protected ResponseEntity<Object> handleExceptionInternal(
    ...
    if (status.is5xxServerError()) {
      log.error(
        "Erro HTTP inesperado [traceId={}] em {}",
        traceId,
        path(request),
        ex
      );
  ...
  private String frameworkMessage(
    ...
    if (
      ex instanceof ResponseStatusException responseStatus &&
    ...
    if (
      body instanceof ProblemDetail problem &&
  ...
  private ResponseEntity<ApiError> build(
    ...
    return ResponseEntity.status(status).body(
      error(status, code, message, path, traceId, violations)
    );
  }
