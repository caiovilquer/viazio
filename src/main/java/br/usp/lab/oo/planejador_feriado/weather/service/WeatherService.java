...
public class WeatherService {
    ...
    public Optional<WeatherSummary> getClimateForWindow(
        ...
        if (!from.isBefore(today) && !to.isAfter(today.plusDays(FORECAST_HORIZON_DAYS))) {
            return summarize(
        ...
        for (int yearsBack = CLIMATOLOGY_YEARS; yearsBack >= 1; yearsBack--) {
            samples.add(new DateWindow(safeMinusYears(from, yearsBack), safeMinusYears(to, yearsBack)));
        }
    ...
    private Optional<WeatherSummary> summarize(
        ...
        int size = Math.min(dates.size(), Math.min(maxTemps.size(), minTemps.size()));
        ...
        for (int i = 0; i < size; i++) {
            ...
            Double rain = precipitation != null && i < precipitation.size() ? precipitation.get(i) : null;
            byDate.put(LocalDate.parse(dates.get(i)), new DailySample((max + min) / 2.0, rain));
        }
        ...
        double avgTemp = samples.stream().mapToDouble(DailySample::temperature).average().orElseThrow();
        double variance = samples.stream()
                .mapToDouble(sample -> Math.pow(sample.temperature() - avgTemp, 2))
                .average()
                .orElse(0.0);
        List<Double> rainValues = samples.stream()
                .map(DailySample::precipitation)
                .filter(value -> value != null)
                .toList();
        double avgRain = rainValues.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
        double rainyProbability = rainValues.isEmpty()
                ? 0.0
                : (double) rainValues.stream().filter(value -> value >= RAINY_DAY_THRESHOLD_MM).count()
                        / rainValues.size();
