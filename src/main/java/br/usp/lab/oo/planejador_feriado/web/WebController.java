...
public class WebController {
  ...
  public String viagem(
    ...
    if (query.isBlank()) {
      model.addAttribute(
        "erro",
        "Informe o destino: código ISO de duas letras (ex.: BR, JP) ou nome em inglês (ex.: japan, france)."
      );
      return "resultado";
    }
    ...
    try {
      ...
      if (isCodeQuery) {
        model.addAttribute(
          "erro",
          "País não encontrado para o código \"" +
      ...
      model.addAttribute(
        "termoBuscado",
        isCodeQuery ? query.toUpperCase(Locale.ROOT) : query
      );
    }
  ...
  public String recomendacoes(
    ...
    try {
      RecommendationResponse resultado = recommendationEngine.recommend(
        request
      );
      ...
      model.addAttribute(
        "erro",
        "Não foi possível gerar as recomendações no momento. Tente novamente em instantes."
      );
      model.addAttribute("fromFormatado", fromDate);
      model.addAttribute("toFormatado", toDate);
    }
  ...
  private String validateRecommendationInput(
    ...
    if (days > RecommendationLimits.MAX_RECOMMENDATION_WINDOW_DAYS) {
      return (
        "A janela máxima permitida é de " +
    ...
    if (limit < 1 || limit > RecommendationLimits.MAX_RESULTS) {
      return (
        "O limite de resultados deve estar entre 1 e " +
