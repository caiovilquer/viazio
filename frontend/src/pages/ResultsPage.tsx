    ...
    <div className="mx-auto max-w-6xl px-4 py-10 pb-28 lg:py-14">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-2xl tracking-tight sm:text-3xl">Seus melhores destinos</h1>
        ...
        <div className="flex gap-2 self-start">
          <Button
            variant="outline"
            size="sm"
            className={cn('gap-2 rounded-full', compareMode && 'border-gold/40 text-gold')}
      ...
      {data?.window && (
        <div className="mb-6 flex items-center gap-4 rounded-2xl border border-hairline bg-surface/50 p-4">
          <ScoreRing score={data.window.score} size={52} strokeWidth={5} label="janela" />
          <div className="min-w-0">
            <p className="text-sm font-medium">Qualidade da janela</p>
            <p className="mt-0.5 text-sm text-muted-foreground">{describeWindow(data.window)}</p>
          </div>
        </div>
      )}
      ...
      {data && data.skipped.length > 0 && (
        <p className="mt-8 text-center text-xs text-muted-foreground">
          {pluralize(data.skipped.length, 'destino ignorado', 'destinos ignorados')} por falta de dados.
        </p>
      )}
