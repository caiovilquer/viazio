...
export function WindowCard({
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.4, delay: Math.min(index * 0.06, 0.36), ease: ease.out }}
      whileHover={{ y: -3 }}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h3 className="text-balance font-display text-lg tracking-tight sm:text-xl">{window.label}</h3>
      ...
      {/* Days at a glance — free days (the gift) vs vacation days (the cost) */}
      <div className="mt-5">
        <div className="flex gap-1">
          {Array.from({ length: window.totalDays }).map((_, i) => (
            <div
              key={i}
              className={cn('h-7 flex-1 rounded-md', i < freeDays ? 'bg-gold/45' : 'bg-foreground/15')}
            />
          ))}
        </div>
        <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          ...
          {window.requiredLeaveDays > 0 && (
            <span className="flex items-center gap-1.5">
              ...
              {pluralize(window.requiredLeaveDays, 'dia de férias', 'dias de férias')}
            </span>
          )}
          {window.bridgeDaysUsed > 0 && (
            <span>· {pluralize(window.bridgeDaysUsed, 'dia de ponte', 'dias de ponte')}</span>
          )}
        </div>
      </div>
          ...
          <div className="flex gap-2.5 overflow-x-auto pb-1 no-scrollbar">
            {window.topDestinations.map((dest) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="max-w-[8rem] truncate text-sm font-medium">{dest.countryName}</p>
                  <p className="text-xs tabular-nums text-gold">{Math.round(dest.tripScore)} pts</p>
                </div>
              </Link>
            ))}
          </div>
