  ...
  return (
    <div className="mb-6 rounded-2xl border border-hairline bg-surface/50 p-4">
      ...
      <div className="mt-3 flex gap-2 overflow-x-auto pb-1 no-scrollbar">
        {profiles.map((profile) => {
          ...
          return (
            ...
            >
              <Icon
                className={cn("size-3.5", isActive && "text-gold")}
                strokeWidth={2.2}
              />
              {profile.label}
            </motion.button>
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          ...
          >
            <SlidersHorizontal
              className={cn("size-3.5", custom && "text-gold")}
              strokeWidth={2.2}
            />
            Personalizar
          </motion.button>
          <SheetContent className="flex flex-col gap-0 overflow-y-auto">
            <SheetHeader>
              ...
              <SheetDescription>
                Ajuste os pesos e veja o ranking dos destinos desta busca se
                reorganizar instantaneamente. Para incluir outros países, ajuste
                a busca.
              </SheetDescription>
            </SheetHeader>
      ...
      <p className="mt-3 flex items-start gap-1.5 text-xs leading-relaxed text-muted-foreground">
        ...
        <span>
          Perfis e pesos apenas reordenam os destinos já encontrados nesta
          busca. Para incluir outros países, use{" "}
          <span className="font-medium text-foreground/80">Ajustar busca</span>.
        </span>
      </p>
    </div>
  );
