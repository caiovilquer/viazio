...
/** Mapeia um valor em [GAUGE_MIN, GAUGE_MAX] para um ângulo do medidor em [-90, 90] (semicírculo superior). */
  ...
  return (
    <div className="flex flex-col items-center">
      <svg
        width={canvasWidth}
        height={size / 2 + 28}
        viewBox={`0 0 ${canvasWidth} ${size / 2 + 28}`}
      >
        <path
        ...
        <text
          x={pointOnArc(cx, cy, r + 14, -90).x}
          y={pointOnArc(cx, cy, r + 14, -90).y + 4}
          textAnchor="middle"
...
export function ClimateChart({ climate }: { climate: ClimateSnapshot }) {
  const dryScore = Math.max(
    0,
    Math.min(100, 100 - climate.rainyDayProbability * 100),
  );
  const rainTier =
    dryScore >= 80
      ? "excellent"
    ...
    <div className="rounded-2xl border border-hairline bg-surface-1/60 p-5 sm:p-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2 sm:items-center">
        <TemperatureGauge
          ...
          tempStdDevC={climate.tempStdDevC}
        />
        ...
        <div className="flex items-center justify-center gap-5 sm:justify-start">
          ...
          <div className="min-w-0">
            <p className="font-display text-lg leading-tight">
              {rainDescription(climate.rainyDayProbability)}
            </p>
            <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
              {Math.round(climate.rainyDayProbability * 100)}% de chance de
              chuva num dia qualquer, ≈{climate.avgDailyPrecipMm.toFixed(1)}
               mm/dia em média.
            </p>
          </div>
        </div>
...
function RainGauge({ score, color }: { score: number; color: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="absolute font-display text-xs font-semibold tabular-nums">
        {Math.round(score)}
      </span>
    </div>
  );
