...
/**
 * Viazio mark — the authored gold "V" (faceted swoosh) crowned by a star.
 * Paths taken from the user's /public/icon.svg, trimmed to the artwork bounds so
 * it locks up tightly with the wordmark. Flat brand gold (#dfae62).
 */
export function BrandMark({ className }: { className?: string }) {
  return (
    <svg viewBox="95 118 895 805" fill="#dfae62" className={cn('h-7 w-7', className)} role="img" aria-label="Viazio">
...
/**
 * Viazio wordmark — geometric, hairline, all-caps, redrawn as vectors to match
 * the authored logo (crossbar-less "A", true-circle "O"). `non-scaling-stroke`
 * keeps the hairline crisp at any size. Inherits color via currentColor.
 */
  ...
  return (
    ...
    >
      <g vectorEffect="non-scaling-stroke">
        ...
        <path d="M188 0 L240 0 L188 100 L240 100" vectorEffect="non-scaling-stroke" />
...
/**
 * Horizontal lockup (mark + wordmark). Scales with font-size, so callers set the
 * overall size with a `text-[…]` class. `showWordmark={false}` → mark only.
 */
