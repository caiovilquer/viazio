...
export type ProfileKey =
  "economico" | "clima-perfeito" | "aventura" | "cultural" | "equilibrado";
...
export interface GroundCostEstimate {
  ...
  travelers: number;
  days: number;
  /** Nível de preço do destino relativo ao Brasil (âncora BRL), 1,0 = igual. */
  relativePriceLevel: number;
  destinationDataYear: string;
  /** Ano dos dados de nível de preço do Brasil — âncora BRL; custo local é independente da origem. */
...
export interface RecommendationResponse {
  ...
  /** Rótulo livre resolvido para exibição (ex.: "padrão", "personalizado",
   *  "economico (ajustado)") — NÃO é um `ProfileKey`. Não alimente de volta um seletor
   *  de perfil ou o parâmetro `profile` da busca sem conferir em
   *  `meta.profiles` primeiro; valor não reconhecido (qualquer coisa que não seja chave
   *  exata de preset) significa que pesos personalizados foram usados. */
  profile: string | null;
  weights: Record<CriterionKey, number>;
...
export interface BestWindowsResponse {
  from: string;
  to: string;
  /** Mesmo rótulo livre resolvido que `RecommendationResponse.profile` — veja lá. */
  profile: string | null;
