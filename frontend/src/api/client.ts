...
export async function fetchRecommendations(request: RecommendationSearchRequest) {
...
export async function fetchBestWindows(query: BestWindowsQuery) {
  const { data } = await api.get<BestWindowsResponse>('/recommendations/best-windows', {
    params: {
      from: query.from,
      to: query.to,
