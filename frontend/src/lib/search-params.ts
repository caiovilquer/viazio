import type { CriterionKey, OriginInput, ProfileKey, RecommendationSearchRequest, Region } from '@/api/types'
...
export function criteriaToSearchParams(criteria: SearchCriteria): URLSearchParams {
  ...
  const weightEntries = Object.entries(criteria.weights).filter(([, v]) => v !== undefined)
  ...
  if (criteria.origin.countryCode) params.set('originCountry', criteria.origin.countryCode)
  if (criteria.origin.city) params.set('originCity', criteria.origin.city)
  if (criteria.origin.latitude !== undefined) params.set('originLat', String(criteria.origin.latitude))
  if (criteria.origin.longitude !== undefined) params.set('originLng', String(criteria.origin.longitude))
  if (criteria.maxGroundBudgetBrl) params.set('maxBudget', String(criteria.maxGroundBudgetBrl))
  return params
}

export function searchParamsToCriteria(params: URLSearchParams): SearchCriteria | null {
  ...
  return {
    ...
    maxGroundBudgetBrl: params.get('maxBudget') ? Number(params.get('maxBudget')) : undefined,
  }
}

export function criteriaToRequest(criteria: SearchCriteria): RecommendationSearchRequest {
  return {
    from: criteria.from,
    to: criteria.to,
    ...
    weights: Object.keys(criteria.weights).length > 0 ? criteria.weights : undefined,
